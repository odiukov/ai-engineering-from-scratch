"""
Примитивная модель мультиагента — эталон.

Открывай ПОСЛЕ своих зелёных тестов.
"""

# Значение handoff, означающее «дальше передавать некому».
DONE = "done"
...
def agents_are_interchangeable(a, b):
    ...
    return (a["system_prompt"] == b["system_prompt"]
            and sorted(a["tools"]) == sorted(b["tools"]))
...
def post(pool, sender, content, handoff=None):
    ...
    """
    message = {"from": sender, "content": content, "handoff": handoff}
    pool.append(message)
    return message
...
def project(pool, agent_name):
    ...
    """
    return [m for m in pool if m["from"] == agent_name or m["handoff"] == agent_name]
...
def run_static(team, pool, order, max_steps=10):
    ...
    for name in order[:max_steps]:
        agent = team[name]
        message = agent["policy"](pool)
        post(pool, name, message["content"], message.get("handoff"))
    return pool
...
def run_handoff(team, pool, start, max_steps=10):
    ...
    for _ in range(max_steps):
        if current not in team:
            return pool
        message = team[current]["policy"](pool)
        nxt = message.get("handoff")
        post(pool, current, message["content"], nxt)
...
def run_selector(team, pool, start, selector, max_steps=10):
    ...
    for _ in range(max_steps):
        if current is None or current not in team:
            return pool
        message = team[current]["policy"](pool)
        post(pool, current, message["content"], message.get("handoff"))
        current = selector(pool, names)
    return pool
