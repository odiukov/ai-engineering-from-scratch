"""
GAN: генератор против дискриминатора — эталон.
...
def binary_cross_entropy(p, target):
    ...
    return -(target * math.log(q) + (1.0 - target) * math.log(1.0 - q))
...
def discriminator_loss(d_real, d_fake):
    ...
    """
    if not d_real or not d_fake:
        raise ValueError("both batches must be non-empty")
    real_term = sum(binary_cross_entropy(p, 1.0) for p in d_real) / len(d_real)
    fake_term = sum(binary_cross_entropy(p, 0.0) for p in d_fake) / len(d_fake)
...
def generator_loss(d_fake, non_saturating=True):
    ...
    """
    if not d_fake:
        raise ValueError("d_fake must be non-empty")
    eps = 1e-12
    total = 0.0
    for p in d_fake:
        q = min(max(p, eps), 1.0 - eps)
        total += -math.log(q) if non_saturating else math.log(1.0 - q)
    return total / len(d_fake)
...
def generator_loss_grad(d_fake, non_saturating=True):
    ...
    n = len(d_fake)
    if non_saturating:
        return [-(1.0 - p) / n for p in d_fake]
    return [-p / n for p in d_fake]
...
def is_mode_collapse(samples, threshold=0.0, min_share=0.1):
    ...
    return min(below, above) / len(samples) < min_share
