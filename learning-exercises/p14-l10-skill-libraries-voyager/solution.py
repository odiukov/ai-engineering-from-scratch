"""
Библиотеки навыков и пожизненное обучение (Voyager) — эталон.

Открывай ПОСЛЕ своих зелёных тестов.

Voyager делает пространством действий не команды, а код: навык — это
именованный кусок кода с описанием. Навыки достаются по похожести описания,
собираются друг из друга и переписываются по фидбеку среды. В библиотеку
навык попадает только после того, как он отработал: self-verification — это
не формальность, а единственное, что отличает библиотеку от свалки.

Соответствие настоящему API (Claude Agent SDK skills, skillkit):

    make_skill         <-  описание skill: name, description, code, tags
    register_skill     <-  запись в реестр ПОСЛЕ self-verification
    search_skills      <-  retrieval по описанию (в Voyager — по эмбеддингу)
    dependency_order   <-  порядок запуска под-навыков (skill DAG)
    compose_skill      <-  сборка нового навыка из найденных примитивов
    execute_skill      <-  прогон в среде и сбор сигнала обратной связи
    propose_next_task  <-  automatic curriculum: чего в библиотеке ещё нет

Ни LLM, ни среды: «код» навыка — строка, среда — обычный словарь, а
исполнение приходит параметром runtime (имя -> функция от env). Библиотека —
словарь name -> навык:

    {"name": "mine_ore", "description": "mine iron ore from rock",
     "code": "mine(3)", "tags": ("gather",), "depends_on": (),
     "version": 1, "history": ()}
"""
...
def make_skill(name, description, code, tags=(), depends_on=()):
    ...
    if not description.strip():
        raise ValueError(f"skill {name!r} has an empty description")
    return {
        "name": name,
...
def register_skill(library, skill, verify):
    ...
    """
    ok, reason = verify(skill)
    if not ok:
        return dict(library), f"rejected {skill['name']}: {reason}"
    updated = dict(library)
    existing = library.get(skill["name"])
    new_skill = dict(skill)
    if existing is None:
        new_skill["version"] = 1
        new_skill["history"] = ()
        message = f"registered {skill['name']} v1"
    else:
        new_skill["version"] = existing["version"] + 1
        new_skill["history"] = tuple(existing["history"]) + (existing["code"],)
        message = f"refined {skill['name']} -> v{new_skill['version']}"
    updated[skill["name"]] = new_skill
...
def search_skills(library, query, top_k=3, tag=None):
    ...
    for name in library:
        skill = library[name]
        if tag is not None and tag not in skill["tags"]:
            continue
        d_tokens = set(skill["description"].lower().split())
        overlap = len(q_tokens & d_tokens)
        if overlap == 0:
            continue
        scored.append((overlap / len(q_tokens | d_tokens), skill))
...
def dependency_order(library, name):
    ...
    def visit(node):
        ...
        if node not in library:
            raise KeyError(node)
        in_progress.add(node)
        for dep in sorted(library[node]["depends_on"]):
            visit(dep)
...
def compose_skill(library, name, description, subskills, tags=()):
    ...
    for sub in subskills:
        if sub not in library:
            raise KeyError(sub)
    code = "; ".join(f"{sub}()" for sub in subskills)
    return make_skill(name, description, code, tags=tags,
                      depends_on=tuple(subskills))
...
def execute_skill(library, name, runtime, env=None):
    ...
    """
    order = dependency_order(library, name)
    state = dict(env or {})
    log = []
    for skill_name in order:
        version = library[skill_name]["version"]
        fn = runtime.get(skill_name)
        if fn is None:
            log.append(f"error in {skill_name} v{version}: no runtime")
            return state, log, False
        try:
            result = fn(state)
        except Exception as exc:  # noqa: BLE001 — это и есть сбор фидбека среды
            log.append(f"error in {skill_name} v{version}: "
                       f"{type(exc).__name__}: {exc}")
            return state, log, False
        log.append(f"ran {skill_name} v{version}: {result}")
...
def propose_next_task(library, wanted, threshold=0.3):
    ...
    for description in wanted:
        hits = search_skills(library, description, top_k=1)
