...
# Байт KV на токен: 4000 токенов ≈ 500 МБ на 70B FP8.
...
def restore_ms(tokens, tier, bytes_per_token=KV_BYTES_PER_TOKEN):
    ...
    restore_ms(4000, "cpu")   ->  10.5    (0.5 + 500 МБ по 50 ГБ/с)
    restore_ms(4000, "disk")  ->  примерно 373.33  (40 + 500 МБ по 1.5 ГБ/с)

    Сравни с recompute_ms(4000) = 500.0: CPU дешевле пересчёта в полсотни раз,
    диск — всего в 1.3 раза, и то не всегда.
    ...
    пересчёт трёхсот токенов. Из-за этого у диска есть длина, ниже которой он
...
def effective_hit_ms(tokens, tier):
    ...
    effective_hit_ms(4000, "disk")  ->  примерно 373.33  (подъём дешевле)
    effective_hit_ms(500, "disk")   ->  62.5             (пересчёт дешевле!)
    effective_hit_ms(4000, "cpu")   ->  10.5

    Диск обгоняет пересчёт начиная с 961 токена: 40 / (0.125 - 0.0833) = 960.
    Ниже этой длины попадание на диске есть, а выигрыша нет — движок молча
    считает заново. Это ровно строчка урока «Short contexts (<1K tokens):
    transfer time > re-prefill», только посчитанная.
