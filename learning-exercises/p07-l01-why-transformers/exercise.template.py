...
def attention_memory_cells(seq_len, n_heads=1, n_layers=1):
    """Сколько ячеек матриц внимания надо держать: n_layers * n_heads * seq_len^2.

    attention_memory_cells(4)                         ->  16
    attention_memory_cells(4, n_heads=8)              ->  128
    attention_memory_cells(8, n_heads=8, n_layers=2)  ->  1024

    Квадрат — это и есть цена трансформера. Удвоил длину контекста — получил
    вчетверо больше памяти. Flash Attention (урок 12) убирает константу, но
    не квадрат.
    """
    raise NotImplementedError
