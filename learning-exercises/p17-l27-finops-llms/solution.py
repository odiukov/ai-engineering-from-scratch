...
def zscore(value, history):
    ...
    zscore(51.0, [50.0] * 5)                       ->  0.0
    zscore(51.0, [50.0])                           ->  0.0

    Две ловушки, и обе дают ложную тревогу:

    1. Нулевое стандартное отклонение. Соблазн написать `sd or 1` — то есть
       заменить ноль единицей — превращает ровный базлайн в детектор с
       фиксированным окном в один доллар: при истории из пяти ровных $50
       спенд $54.20 даёт z = 4.2 и будит on-call из-за роста на 8%.
    2. История короче двух значений. Дисперсии ещё нет, аномалий тоже.

    В обоих случаях честный ответ — 0.0.
    """
    n = len(history)
    if n < 2:
        return 0.0
