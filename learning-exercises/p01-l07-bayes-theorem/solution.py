...
def beta_map(params):
    ...
    Формула (a - 1) / (a + b - 2). Ловушка: при a = b = 1 знаменатель ноль —
    у равномерного распределения моды нет, договоримся возвращать 0.5.
    ...
    denominator = a + b - 2
    if denominator <= 0:
        # равномерная Beta(1,1): плотность плоская, максимум не единственный
        return 0.5
