...
def dense_params(in_features, out_features, bias=True):
    ...
    return total + out_features if bias else total
...
def lenet5_shapes(input_size=32):
    ...
    """
    size = input_size
    shapes = [("input", (1, size, size))]
    size = spatial_out(size, 5)
    shapes.append(("conv1", (6, size, size)))
    size = spatial_out(size, 2, stride=2)
    shapes.append(("pool1", (6, size, size)))
    size = spatial_out(size, 5)
    shapes.append(("conv2", (16, size, size)))
    size = spatial_out(size, 2, stride=2)
    shapes.append(("pool2", (16, size, size)))
    return shapes
...
def lenet5_params():
    ...
    flat = c * h * w
    return [
        ("conv1", conv_params(1, 6, 5)),
        ("conv2", conv_params(6, 16, 5)),
        ("fc1", dense_params(flat, 120)),
        ("fc2", dense_params(120, 84)),
        ("fc3", dense_params(84, 10)),
    ]
...
def shortcut_kind(in_c, out_c, stride):
    ...
    """
    return "identity" if stride == 1 and in_c == out_c else "projection"
...
def residual_forward(x, f):
    ...
    """
    return [a + b for a, b in zip(f(x), x)]
...
def gradient_scale(gains, residual=False):
    ...
    scale = 1.0
    for g in gains:
        scale *= (g + 1.0) if residual else g
    return scale
