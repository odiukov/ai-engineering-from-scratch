"""
RLHF: reward model и PPO — эталон.
...
def sigmoid(x):
    ...
    """
    # две ветки, а не одна формула: обе алгебраически равны, но каждая
    # безопасна только на своей половине оси
    if x >= 0:
...
def bradley_terry_loss(reward_chosen, reward_rejected):
    ...
    # softplus-форма: log(1 + e^(-d)). При d < 0 выносим e^(-d) за скобку,
    # иначе экспонента переполняется, а лосс здесь заведомо конечен
    if diff >= 0:
...
def bradley_terry_grad(reward_chosen, reward_rejected):
    ...
    """
    s = sigmoid(reward_chosen - reward_rejected)
    g = s - 1.0
    return (g, -g)
...
def reward_model_accuracy(scored_pairs):
    ...
    if not scored_pairs:
        return 0.0
    correct = sum(1 for chosen, rejected in scored_pairs if chosen > rejected)
...
def kl_divergence(p, q):
    ...
    for pi, qi in zip(p, q):
        if pi <= 0.0:
            # предел x*log(x) при x -> 0 равен нулю, а math.log(0) упадёт
            continue
...
def rlhf_objective(reward, policy_logits, ref_logits, beta=0.02):
    ...
    """
    p = softmax(policy_logits)
    q = softmax(ref_logits)
