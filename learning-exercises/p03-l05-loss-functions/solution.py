...
def bce_gradient(predictions, targets, eps=1e-15):
    ...
    Тот же зажим в [eps, 1 - eps], иначе деление на ноль.
    И то же деление на n, что в mse_gradient: loss усредняется по батчу,
    значит и градиент обязан. Расхождение здесь — тихий баг, который
    численная проверка градиента ловит сразу.

    Смысл: при t = 1 и крошечном p градиент равен -1/p, то есть огромен.
    Уверенно ошибся — получи соответствующий сигнал. У MSE в этом месте
    градиент, наоборот, упирается в потолок -2.
    """
    n = len(predictions)
    grads = []
    for p, t in zip(predictions, targets):
