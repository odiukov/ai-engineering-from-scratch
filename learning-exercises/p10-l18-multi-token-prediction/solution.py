"""
Multi-Token Prediction (MTP) — эталон.

Открывай ПОСЛЕ своих зелёных тестов.

DeepSeek-V3 добавил к обычному «предскажи следующий токен» ещё D потерь:
предскажи токен через два, через три и так далее. Модули последовательные,
а не параллельные: каждый следующий строится из предыдущего скрытого
состояния И эмбеддинга уже известного токена. Именно эта цепочка делает
модули пригодными как черновик для спекулятивного декодирования.

Формулы урока:
    h^(0)  = выход основной модели на позиции i
    h^(k)  = T_k( M_k * concat(rms_norm(h^(k-1)), rms_norm(E(t_{i+k}))) )
    logits для глубины k = Out(h^(k-1)),  k = 1..D
    L_k    = CE(logits, t_{i+k})
    L_MTP  = (lambda / D) * sum_k L_k

Embedding — список строк по числу токенов словаря. Матрица — список строк.
"""
...
def matvec(M, x):
    ...
    """
    if M and len(M[0]) != len(x):
        raise ValueError(f"матрица {len(M)}x{len(M[0])} не умножается на вектор {len(x)}")
...
def depth_hidden(prev_hidden, token_embed, M):
    ...
    """
    joined = rms_norm(prev_hidden) + rms_norm(token_embed)
    return matvec(M, joined)
...
def mtp_depth_losses(h0, targets, projections, embedding, W_out):
    ...
    """
    depth = len(targets)
    if depth > 1 and len(projections) < depth - 1:
        raise ValueError(f"нужно минимум {depth - 1} проекций, дано {len(projections)}")

    losses = []
    h = list(h0)
    for k in range(depth):
        losses.append(cross_entropy(matvec(W_out, h), targets[k]))
        if k + 1 < depth:
            # следующая глубина видит уже раскрытый токен текущей
            h = depth_hidden(h, embedding[targets[k]], projections[k])
    return losses
...
def joint_loss(main_loss, depth_losses, lam):
    ...
    if not depth_losses:
        return main_loss
    return main_loss + lam / len(depth_losses) * sum(depth_losses)
...
def mtp_extra_params(hidden, depths=1, ffn_hidden=None):
    ...
    if ffn_hidden is None:
        ffn_hidden = 8 * hidden // 3
    projection = 2 * hidden * hidden
    attention = 4 * hidden * hidden
    mlp = 3 * hidden * ffn_hidden
    per_module = projection + attention + mlp
    return {
