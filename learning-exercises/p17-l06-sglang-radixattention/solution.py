"""
SGLang и RadixAttention: префиксное дерево KV-кэша и cache-aware планировщик — эталон.

Открывай ПОСЛЕ своих зелёных тестов.
"""

# Политики планировщика из урока.
POLICIES = ("fcfs", "cache_aware")

# Компоненты промпта. Числа — токены; важны только совпадения, не значения.
# Неизменяемая часть (system, tools) идёт первой, динамическая — последней.
PARTS = {
...
# Канонический порядок сборки промпта — он же ключ кэша.
CANONICAL_ORDER = ("system", "tools", "context_a")

# Тот же набор компонентов, переставленный местами. Человек видит тот же
# промпт, префиксное дерево — другую последовательность.
SCRAMBLED_ORDER = ("system", "context_a", "tools")

# Уникальный вопрос на каждый запрос: 20 токенов, ни с кем не общих.
QUESTIONS = tuple(tuple(range(9000 + 100 * i, 9020 + 100 * i)) for i in range(8))

# Восемь запросов с одинаковым префиксом на 400 токенов — RAG в чистом виде.
ORDERED_WORKLOAD = tuple(
    PARTS["system"] + PARTS["tools"] + PARTS["context_a"] + QUESTIONS[i]
    for i in range(8)
)

# Тот же трафик, но клиент собирает промпт то так, то эдак.
...
# Две ветки документов вперемешку: A, B, A, B... На этом трафике и видно
# разницу между FCFS и cache-aware, когда кэш меньше обеих веток сразу.
...
# Ёмкость кэша в токенах: общий префикс (320) плюс одна ветка документа (80)
# плюс несколько вопросов. Обе ветки одновременно не помещаются — ровно та
# ситуация, в которой политика планировщика перестаёт быть формальностью.
CACHE_CAPACITY = 480
...
def render_prompt(order, parts):
    ...
    """
    if not order:
        raise ValueError("empty order: a prompt needs at least one component")
    tokens = ()
    for name in order:
        if name not in parts:
            raise KeyError(name)
        tokens += tuple(parts[name])
    return tokens
...
class RadixCache:
    ...
    def __init__(self, capacity_tokens):
        ...
        if capacity_tokens < 1:
            raise ValueError("capacity_tokens must be at least 1")
        self.capacity_tokens = capacity_tokens
        # корень несёт пустой отрезок и не вытесняется никогда
        self._root = {"segment": (), "children": {}, "parent": None, "last_used": 0.0}
        self._used = 0
    ...
    def match(self, tokens):
        ...
        while i < len(tokens):
            ...
            if child is None:
                break
            m = common_prefix_len(child["segment"], tokens[i:])
            i += m
            if m < len(child["segment"]):
                break   # ушли в середину отрезка — глубже идти уже некуда
            node = child
        return i
    ...
    def insert(self, tokens, now):
        ...
        if len(tokens) > self.capacity_tokens:
            raise CacheTooSmall(
                f"prompt of {len(tokens)} tokens exceeds capacity {self.capacity_tokens}"
            )
        tokens = tuple(tokens)
        node = self._root
        node["last_used"] = now
        ...
        while i < len(tokens):
            ...
            if child is None:
                break
            seg = child["segment"]
            m = common_prefix_len(seg, tokens[i:])
            if m < len(seg):
                # расхождение внутри отрезка: узел режется надвое, хвост
                # уезжает в новый узел-потомок вместе со всеми детьми.
                # Метку времени хвост УНАСЛЕДУЕТ старую: этот запрос по нему
                # не прошёл, и горячим он от чужого совпадения не стал.
                tail = {"segment": seg[m:], "children": child["children"],
                        "parent": child, "last_used": child["last_used"]}
                for grandchild in tail["children"].values():
                    grandchild["parent"] = tail
                child["segment"] = seg[:m]
                child["children"] = {seg[m]: tail}
            child["last_used"] = now
            i += m
            node = child
        reused = i
        new = len(tokens) - reused
        if new:
            if self._used + new > self.capacity_tokens:
                self.evict(self._used + new - self.capacity_tokens, now)
            leaf = {"segment": tokens[reused:], "children": {},
                    "parent": node, "last_used": now}
            node["children"][tokens[reused]] = leaf
            self._used += new
        return {"reused": reused, "new": new}
    ...
    def evict(self, need_tokens, now):
        ...
        while freed < need_tokens:
            ...
            while stack:
                node, path = stack.pop()
                path += tuple(node["segment"])
                for child in node["children"].values():
                    stack.append((child, path))
                if node is self._root or node["children"] or node["last_used"] >= now:
                    continue
                key = (node["last_used"], path)
                if victim is None or key < (victim["last_used"], victim_path):
                    victim, victim_path = node, path
            if victim is None:
                raise CacheTooSmall(
                    f"cannot free {need_tokens} tokens: no evictable leaf left"
                )
            del victim["parent"]["children"][victim["segment"][0]]
            self._used -= len(victim["segment"])
            freed += len(victim["segment"])
        return freed
...
def cache_aware_order(cache, prompts, pending):
    ...
    """
    return sorted(pending, key=lambda i: (-cache.match(prompts[i]), i))
...
def run_workload(prompts, capacity_tokens, policy):
    ...
    cache = RadixCache(capacity_tokens)
    ...
    now = 0.0
    while pending:
        i = pending[0] if policy == "fcfs" else cache_aware_order(cache, prompts, pending)[0]
        pending.remove(i)
        now += 1.0     # время — параметр, а не time.time(): прогон обязан повторяться
        stats = cache.insert(prompts[i], now)
        order.append(i)
        reused_total += stats["reused"]
        new_total += stats["new"]
    prompt_tokens = reused_total + new_total
    hit_rate = reused_total / prompt_tokens if prompt_tokens else 0.0
    return {
        "order": order,
        "reused_tokens": reused_total,
        "prefill_tokens": new_total,
        "prompt_tokens": prompt_tokens,
        "hit_rate": hit_rate,
        "speedup": prefill_speedup(hit_rate),
    }
