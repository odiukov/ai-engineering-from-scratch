"""
Prompt engineering: шаблоны, ограничения, оценка — эталон.
...
# Каталог паттернов из урока. Заготовка получает его целиком, поэтому тесты
# могут импортировать PROMPT_PATTERNS из exercise и не дублировать данные.
PROMPT_PATTERNS = {
    "persona": {
...
# Тег-делимитер, в который заворачивается недоверенный пользовательский ввод.
DELIMITER_TAG = "user_input"

# Сигнатуры prompt injection. Регистр не важен, поэтому re.IGNORECASE.
INJECTION_PATTERNS = (
...
def template_variables(template):
    ...
    """
    seen = []
    # findall по одной группе вернёт список имён; порядок — как в тексте
...
def render_template(template, variables):
    ...
    """
    missing = [v for v in template_variables(template) if v not in variables]
    if missing:
        raise ValueError(f"missing variables: {sorted(missing)}")

    # re.sub с функцией: одна проходка по строке, чужие скобки не трогаем
    def _swap(match):
        return str(variables[match.group(1)])

    return re.sub(r"\{([A-Za-z_][A-Za-z0-9_]*)\}", _swap, template)
...
def build_prompt(pattern_name, variables, system_override=None):
    ...
    if pattern is None:
        raise ValueError(
            f"unknown pattern: {pattern_name}. available: {sorted(PROMPT_PATTERNS)}"
        )
    # render_template сам бросит ValueError, если переменных не хватает
    user = render_template(pattern["template"], variables)
...
def score_response(text, criteria):
    ...
    if "max_words" in criteria:
        ...
        scores["length_compliant"] = words <= criteria["max_words"]

    if "required_keywords" in criteria:
        wanted = criteria["required_keywords"]
        found = [kw for kw in wanted if kw.lower() in lowered]
        scores["keywords_found"] = found
        # пустой список требований — покрытие полное, а не деление на ноль
        scores["keyword_coverage"] = len(found) / len(wanted) if wanted else 1.0

    if "forbidden_phrases" in criteria:
        bad = [p for p in criteria["forbidden_phrases"] if p.lower() in lowered]
        scores["forbidden_violations"] = bad
        scores["no_violations"] = not bad
    ...
    if "expected_format" in criteria:
        ...
        if fmt == "json":
            ...
            valid = len([ln for ln in lines if re.match(r"^\d+[.)]", ln)]) >= 2
        else:
            # неизвестный формат не должен ронять оценку остальных критериев
            valid = True
        scores["format_valid"] = valid
...
def composite_score(scores):
    ...
    for value in scores.values():
        if isinstance(value, bool):
            parts.append(1.0 if value else 0.0)
...
def rank_models(responses, criteria):
    ...
    """
    scored = [
        (name, composite_score(score_response(text, criteria)))
        for name, text in responses.items()
    ]
    # сортировка стабильна: сначала по имени, потом по баллу убыв.
