"""
Стриминговый speech-to-speech: Moshi и Hibiki — эталон.
...
def frame_ms_from_rate(frame_rate_hz):
    ...
    """
    if frame_rate_hz <= 0:
        raise ValueError("frame_rate_hz должен быть положительным")
...
def tokens_per_second(frame_rate_hz, codebooks, streams=1):
    ...
    """
    if frame_rate_hz <= 0 or codebooks <= 0 or streams <= 0:
        raise ValueError("все параметры должны быть положительными")
    return float(frame_rate_hz) * codebooks * streams
...
def theoretical_latency_ms(frame_ms=80.0, acoustic_delay_frames=1):
    ...
    """
    if frame_ms <= 0 or acoustic_delay_frames < 0:
        raise ValueError("некорректные параметры задержки")
    return float(frame_ms) * (1 + acoustic_delay_frames)
...
def pipeline_latency_ms(stage_ms):
    ...
    for ms in stage_ms:
        if ms < 0:
            raise ValueError("стадия не может занимать отрицательное время")
        total += ms
    return total
...
def build_frame(text_token, acoustic_tokens, n_codebooks=8):
    ...
    """
    if len(acoustic_tokens) != n_codebooks:
        raise ValueError("ожидалось ровно n_codebooks акустических токенов")
    # list() ради копии: кадр уезжает в историю сессии и не должен
    # меняться вслед за буфером, из которого пришёл
    return [text_token] + list(acoustic_tokens)
...
def depth_decode(context, heads):
    ...
    """
    if not heads:
        raise ValueError("нужна хотя бы одна голова кодбука")
    tokens = []
    for head in heads:
        # копия списка: голова не должна иметь возможности испортить
        # уже предсказанное — это тихая ошибка, которую не видно в логах
        tokens.append(head(context, list(tokens)))
    return tokens
...
class DuplexSession:
    ...
    def __init__(self, frame_ms=80.0, n_codebooks=8, pad_token=""):
        ...
        if frame_ms <= 0 or n_codebooks <= 0:
            raise ValueError("некорректные параметры сессии")
    ...
    def step(self, user_acoustic, own_text, own_acoustic):
        ...
        if len(user_acoustic) != self.n_codebooks:
            raise ValueError("кадр пользователя не той ширины")
        # оба потока растут в одном вызове — это и есть full-duplex
        self.user_stream.append(list(user_acoustic))
        frame = build_frame(own_text, own_acoustic, self.n_codebooks)
        self.own_stream.append(frame)
        self.text_stream.append(own_text)
        return frame

    def elapsed_ms(self):
        """Сколько времени прошло по сессии: кадры на длительность кадра."""
        return len(self.own_stream) * self.frame_ms

    def transcript(self):
        """Внутренний монолог как текст: непустые токены через пробел."""
        return " ".join(t for t in self.text_stream if t != self.pad_token)
