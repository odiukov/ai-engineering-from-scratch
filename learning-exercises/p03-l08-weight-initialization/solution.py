...
def kaiming_init(fan_in, fan_out, seed=0):
    ...
    Именно это PyTorch делает по умолчанию в nn.Linear.
    """
...
def recommend_init(activation_name):
    ...
    Правило простое: у всего ReLU-подобного (relu, leaky_relu, gelu, swish)
    половина выходов зануляется, нужна поправка Kaiming. Остальное —
    насыщающиеся сигмоиды, им хватает Xavier. Xavier же и разумный
    запасной вариант для незнакомой активации.
    """
