"""
Self-Refine и CRITIC — эталон.

Открывай ПОСЛЕ своих зелёных тестов.

Здесь собирается руками петля generate -> feedback/verify -> refine из
статей Self-Refine (Madaan et al., 2023) и CRITIC (Gou et al., 2023). В
настоящих фреймворках это Anthropic evaluator-optimizer и
`output_guardrails` в OpenAI Agents SDK. Ни сети, ни LLM тут нет: модель
заменена детерминированной заглушкой, потому что проверяем мы логику
петли, а не удачу генерации.
"""
...
def format_issues(output, max_bullets=3, max_chars=60):
    ...
    issues = []
    # порядок замечаний фиксирован, иначе критика будет прыгать от запуска
    # к запуску и петля не сойдётся
    if len(bullets) != len(lines):
        issues.append("non-bullet line present")
    if len(bullets) != max_bullets:
        issues.append(f"expected {max_bullets} bullets, got {len(bullets)}")
    for ln in bullets:
        if len(ln) > max_chars:
            issues.append(f"bullet too long: {len(ln)} > {max_chars}")
    return issues
...
def self_feedback(output):
    ...
    """
    issues = format_issues(output)
    if not issues:
        return ("no issues", True)
    return ("; ".join(issues), False)
...
def external_verify(output, wrong_facts):
    ...
    """
    text = output.lower()
    issues = list(format_issues(output))
    for fact in wrong_facts:
        # факт нарушен, только если совпали все части: одно слово "paris"
        # само по себе ошибкой не является
        if all(part.lower() in text for part in fact):
            issues.append("contradicts reference: " + " + ".join(fact))
    if not issues:
        return ("verifier: ok", True)
    return ("verifier: " + "; ".join(issues), False)
...
def refine_prompt(task, history):
    ...
    for attempt in history:
        parts.append(f"ATTEMPT {attempt['iteration']}:\n{attempt['output']}")
        parts.append(f"CRITIQUE {attempt['iteration']}: {attempt['critique']}")
    if history:
        parts.append("Write an improved version that fixes every critique above.")
    else:
        parts.append("Write the first draft.")
    return "\n".join(parts)
...
def scripted_generate(prompt, script):
    ...
    low = prompt.lower()
    for keyword, draft in script:
        if keyword.lower() in low:
            return draft
    # сюда попадаем, только если пары по умолчанию в скрипте нет
    return script[-1][1]
...
def refine_loop(task, script, verify, max_iterations=4):
    ...
    while True:
        ...
        output = scripted_generate(prompt, script)
        critique, ok = verify(output)
        # повтор считаем только среди уже отвергнутых: принятый черновик и
        # так остановит петлю по 'verified'
        repeated = any(a["output"] == output for a in history)
        stop, reason = should_stop(iteration, ok, repeated, max_iterations)
        history.append(
            {
                "iteration": iteration,
                "prompt": prompt,
...
def loop_report(history):
    ...
    return {
        ...
        "converged": last["verified"],
        "reason": last["stop_reason"],
        "final_output": last["output"],
        "critiques": [a["critique"] for a in history],
    }
