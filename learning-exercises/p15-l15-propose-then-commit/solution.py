...
отвергается, повторный commit не выполняет побочный эффект второй раз.
...
def commit(store, key, execute, now):
    """Выполняет одобренное действие ровно один раз. Возвращает строку-исход.
    ...
    execute — функция (action, payload) с побочным эффектом. Возможные исходы:
    ...
    Ключевая ловушка: статус пишется ПОСЛЕ вызова execute. Если поменять
    порядок, сбой между записью и вызовом оставит запись «выполнено» при
    невыполненном действии, и повтор его никогда не доделает. Оба порядка
    плохи по-своему; поэтому за commit обязательно идёт verify.
    """
    record = store[key]
    ...
    execute(record["action"], record["payload"])
