"""
Guardrails: проверки входа, проверки выхода и конвейер вокруг модели — эталон.
...
# Порог блокировки: проверка с уверенностью ниже него пропускает запрос.
BLOCK_THRESHOLD = 0.75

# Токен: слово или число, дефис и апостроф внутри слова сохраняются.
...
# (регулярка, уверенность). Уверенность >= BLOCK_THRESHOLD блокирует запрос.
INJECTION_PATTERNS = (
...
# Уверенность приёмов маскировки ниже порога: сама по себе кодировка ещё не
# атака, но в сумме с другой находкой она поднимает общий счёт.
...
# Порядок замены важен: телефонная регулярка жаднее остальных, и если пустить
# её первой, она откусит кусок от того, что должно было стать SSN или картой.
REDACTIONS = (
    ("ssn", "[SSN REDACTED]"),
...
def check_length(text, max_chars=5000, max_words=1000):
    ...
    passed = chars <= max_chars and words <= max_words
    return {
        "name": "length",
        "passed": passed,
...
def detect_injection(text):
    ...
    for pattern, pattern_confidence in INJECTION_PATTERNS:
    ...
    if evasion:
        matches.append("encoding_evasion")
        confidence = max(confidence, EVASION_CONFIDENCE)

    return {
        "name": "injection",
        "passed": confidence < BLOCK_THRESHOLD,
...
def detect_pii(text):
    ...
    found = []
    confidence = 0.0
    for pii_type, (pattern, pattern_confidence) in PII_PATTERNS.items():
...
def redact_pii(text):
    ...
    replaced = []
    scrubbed = text
    for pii_type, placeholder in REDACTIONS:
        pattern = PII_PATTERNS[pii_type][0]
        scrubbed, count = re.subn(pattern, placeholder, scrubbed, flags=re.IGNORECASE)
        replaced.extend([pii_type] * count)
...
def classify_topic(text):
    ...
    for category, keywords in TOPIC_KEYWORDS.items():
        hits = [kw for kw in keywords if re.search(r"\b" + re.escape(kw), lowered)]
        if hits:
            flagged.append({"category": category, "keywords": hits})
            confidence = max(confidence, min(0.75 + 0.05 * (len(hits) - 1), 0.99))
...
def check_relevance(question, answer, threshold=0.15):
    ...
    """
    q_words = set(WORD_RE.findall(question.lower())) - STOP_WORDS
    a_words = set(WORD_RE.findall(answer.lower())) - STOP_WORDS
    ...
    return {
        ...
        "details": {"overlap": overlap, "shared": sorted(shared)},
    }
...
def detect_prompt_leak(answer, system_prompt, threshold=0.4):
    ...
    """
    sys_words = set(WORD_RE.findall(system_prompt.lower())) - STOP_WORDS
    if not sys_words:
        return {
            "name": "prompt_leak",
            "passed": True,
            "confidence": 0.0,
            "details": {"similarity": 0.0, "threshold": threshold},
        }

    answer_words = set(WORD_RE.findall(answer.lower()))
    similarity = len(sys_words & answer_words) / len(sys_words)
    return {
        "name": "prompt_leak",
...
def run_guardrails(user_input, model_fn, system_prompt=DEFAULT_SYSTEM_PROMPT, max_chars=5000):
    ...
    """
    report = {"input_checks": [], "output_checks": [], "blocked": False, "reason": ""}

    input_checks = (
        lambda: check_length(user_input, max_chars),
        lambda: detect_injection(user_input),
        lambda: detect_pii(user_input),
        lambda: classify_topic(user_input),
    )
    for make_check in input_checks:
        result = make_check()
        report["input_checks"].append(result)
        if not result["passed"]:
            report["blocked"] = True
            report["reason"] = f"input:{result['name']} (confidence={result['confidence']:.2f})"
            return BLOCKED_INPUT_MESSAGE, report

    model_output = model_fn(user_input)

    output_checks = (
        lambda: classify_topic(model_output),
        lambda: check_relevance(user_input, model_output),
        lambda: detect_prompt_leak(model_output, system_prompt),
    )
    for make_check in output_checks:
        result = make_check()
        report["output_checks"].append(result)
        if not result["passed"]:
            report["blocked"] = True
            report["reason"] = f"output:{result['name']} (confidence={result['confidence']:.2f})"
            return BLOCKED_OUTPUT_MESSAGE, report

    scrubbed, replaced = redact_pii(model_output)
    report["output_checks"].append(
        {
            "name": "pii_redaction",
            "passed": not replaced,
            "confidence": 0.95 if replaced else 0.0,
            "details": replaced,
        }
    )
