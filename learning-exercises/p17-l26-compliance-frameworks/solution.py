"""
Compliance: матрица контролей, полей и сроков хранения — эталон.
...
# Один контроль закрывает пункты сразу нескольких фреймворков — ради этого
# матрицу и строят. Значение: фреймворк -> номер пункта в нём.
CONTROL_MAP = {
    "access logging": {
...
# Что требует закупка в зависимости от географии и сегмента.
PROFILE_MAP = {
...
# Сколько держать аудит-журнал. SOC 2 — год, HIPAA — шесть лет.
RETENTION_DAYS = {
    "SOC 2": 365,
    "GDPR": 365,
    "PCI DSS": 365,
...
# Что именно обязано лежать в записи журнала ради каждого фреймворка.
FIELD_REQUIREMENTS = {
...
def frameworks_for(control, control_map=CONTROL_MAP):
    ...
    return sorted(control_map[control])
...
def required_controls(frameworks, control_map=CONTROL_MAP):
    ...
    return sorted(c for c, mapping in control_map.items() if wanted & set(mapping))
...
def coverage_gaps(implemented, frameworks, control_map=CONTROL_MAP):
    ...
    gaps = {}
    for control in required_controls(frameworks, control_map):
        if control in done:
            continue
        for framework in control_map[control]:
            if framework in frameworks:
                gaps.setdefault(framework, []).append(control)
    return {f: sorted(v) for f, v in gaps.items()}
...
def required_log_fields(frameworks, field_requirements=FIELD_REQUIREMENTS):
    ...
    for framework in frameworks:
        if framework not in field_requirements:
            raise ValueError("unknown framework: %r" % (framework,))
        fields |= set(field_requirements[framework])
...
def record_is_complete(record, frameworks, field_requirements=FIELD_REQUIREMENTS):
    ...
    missing = [
        field
        for field in required_log_fields(frameworks, field_requirements)
        # None и пустая строка — это «поля нет». False и 0.0 — это значения,
        # поэтому проверять на truthiness нельзя
...
def records_to_purge(records, frameworks, now, table=RETENTION_DAYS):
    ...
    """
    limit = retention_days(frameworks, table)
    today = date.fromisoformat(now[:10])
    return [r for r in records if (today - date.fromisoformat(r["ts"][:10])).days > limit]
