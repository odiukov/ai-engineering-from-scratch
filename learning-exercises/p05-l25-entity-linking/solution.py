"""
Entity linking и разрешение неоднозначности — эталон.
...
def tokenize(text):
    ...
    """
    # findall по классу "буква или цифра" сразу и режет, и выбрасывает
    # пунктуацию — отдельный проход strip() не нужен
...
def build_alias_index(pairs):
    ...
    """
    index = {}
    for alias, entity_id in pairs:
        bucket = index.setdefault(alias.strip().lower(), [])
        # линейный поиск по bucket, а не set: списков на алиас 10-30 штук,
        # а порядок (prior) нам важнее скорости проверки
...
def candidates(alias_index, mention):
    ...
    """
    return list(alias_index.get(mention.strip().lower(), []))
...
def disambiguate(mention, context, alias_index, descriptions):
    ...
    """
    pool = candidates(alias_index, mention)
    if not pool:
        return (None, 0.0)
    context_tokens = tokenize(context)
    best, best_score = None, -1.0
    for entity_id in pool:
        score = jaccard(context_tokens, tokenize(descriptions.get(entity_id, "")))
        # строго больше: равенство оставляет победителем более раннего,
        # то есть более популярного кандидата
        if score > best_score:
...
def link_with_nil(mention, context, alias_index, descriptions, threshold=0.05):
    ...
    """
    entity_id, score = disambiguate(mention, context, alias_index, descriptions)
    if entity_id is None or score < threshold:
        return None
    return entity_id
...
def mention_recall(examples, alias_index):
    ...
    if not examples:
        return 0.0
    hits = sum(1 for surface, _, gold in examples if gold in candidates(alias_index, surface))
...
def evaluate_linker(examples, alias_index, descriptions):
    ...
    """
    if not examples:
        return {"mention_recall": 0.0, "disambiguation_accuracy": 0.0, "pipeline_accuracy": 0.0}

    recalled = 0
    disambiguated = 0
    for surface, context, gold in examples:
        pool = candidates(alias_index, surface)
        if gold not in pool:
            # кандидат-генератор не выдал правильный ответ: этот пример
            # выпадает и из recall, и из знаменателя disambiguation
            continue
        recalled += 1
        predicted, _ = disambiguate(surface, context, alias_index, descriptions)
        if predicted == gold:
            disambiguated += 1

    return {
        "mention_recall": recalled / len(examples),
        "disambiguation_accuracy": (disambiguated / recalled) if recalled else 0.0,
        "pipeline_accuracy": disambiguated / len(examples),
    }
