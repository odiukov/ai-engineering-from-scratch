...
Две ручки, которые стоит назвать. `ignore_index=0` пропускает лосс на padding-токенах. `teacher_forcing_ratio` — вероятность взять на шаге настоящий токен вместо предсказания модели. Начинайте с 1.0 (полный teacher forcing) и по ходу обучения снижайте примерно до 0.5, чтобы закрыть разрыв exposure bias.
...
Обучите модель на игрушечной задаче копирования: вход `[a, b, c, d, e]`, выход `[a, b, c, d, e]`. Увеличивайте длину последовательности. Смотрите на точность.
...
Один hidden state GRU не может без потерь запомнить вход из 40 токенов. Информация есть на каждом шаге энкодера, но decoder видит только последнее состояние. Attention чинит это напрямую.

> 🎒 **На пальцах.** Задача элементарная: повтори то, что услышал. При длине 5 модель справляется в 98% случаев, при 40 — в 23%. Длина выросла в 8 раз, точность упала вчетверо. И это на копировании, где вообще не нужно ничего понимать. Если так рушится копирование, представьте, что происходит с переводом.
