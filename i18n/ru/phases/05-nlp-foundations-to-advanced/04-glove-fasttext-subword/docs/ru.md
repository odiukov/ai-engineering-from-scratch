...
> 🎒 **На пальцах.** Разберём `<where>` вручную. Триграммы, то есть кусочки по 3 символа: `<wh`, `whe`, `her`, `ere`, `re>` — ровно пять, потому что в строке из 7 символов ровно 7 − 3 + 1 = 5 позиций для окна длины 3. Дальше то же самое для длин 4, 5 и 6. Плюс само слово целиком.
...
> 🎒 **На пальцах.** Посчитаем первое слияние на этом корпусе руками. Пара `e`+`s` встречается в `newest` (6 раз) и в `widest` (3 раза) — итого 9. Пара `l`+`o` встречается в `low` (5) и `lower` (2) — итого 7. Девять больше семи, поэтому первым сливается `es`. Дальше к нему приклеится `t`, потом `</w>` — так и рождается токен `est</w>`, который вы видите в ответе для слова `lowest`.
