...
Это и есть ядро метрики faithfulness в RAGAS. Разбейте сгенерированный ответ на атомарные утверждения. Проверьте каждое против найденного контекста. Верните долю тех, что дают entailment.

> 🎒 **На пальцах.** Порог 0,5 здесь — это просто «модель считает entailment более вероятным, чем нет». Если ответ разбился на 6 утверждений и 5 прошли проверку, faithfulness = 5/6 ≈ 0,83. Одно непрошедшее — и есть кандидат в галлюцинации, который стоит показать человеку.
