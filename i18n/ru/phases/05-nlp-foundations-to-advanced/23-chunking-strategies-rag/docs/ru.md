...
Ключевая деталь: родителей надо дедуплицировать. Несколько детей могут указывать на одного родителя, и вернуть его несколько раз — значит впустую потратить контекст.

> 🎒 **На пальцах.** Множество `seen` тут делает всю работу. При `top_k=3` три лучших дочерних чанка вполне могут лежать в одном абзаце — без дедупликации вы отправите модели один и тот же родитель три раза и займёте 6 144 токена вместо 2 048. Модель получит одну мысль трижды и ноль новой информации.
