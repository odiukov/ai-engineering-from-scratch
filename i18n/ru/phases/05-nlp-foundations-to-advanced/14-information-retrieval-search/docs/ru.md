...
1. **Sparse retrieval (BM25).** Быстрый, точный на дословных совпадениях, беспомощный в семантике. Работает поверх inverted index. Меньше 10 мс на запрос по миллионам документов. Именно он правильно находит номера статей, артикулы товаров, тексты ошибок и имена собственные.
...
> 🎒 **На пальцах.** Сравните числа: BM25 — до 10 мс, dense — 50-200 мс, то есть в 5-20 раз дольше. А cross-encoder такой тяжёлый, что по миллиону документов его не запустишь никогда. Поэтому его пускают на 30 пар: 30 сравнений вместо 1 000 000: в 33 тысячи раз меньше работы.
...
> 🎒 **На пальцах.** Проследите за размерами. `pool_size=30`: BM25 даёт 30 кандидатов, dense даёт 30, RRF сводит их (уникальных получится от 30 до 60) и снова обрезает до 30. Потом собираются 30 пар «запрос-документ», cross-encoder их оценивает, и наружу выходит `top_k=5`. Из миллиона до пяти за три шага.
