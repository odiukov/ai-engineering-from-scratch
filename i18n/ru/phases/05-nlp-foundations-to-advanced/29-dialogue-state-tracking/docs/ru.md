...
Instructor плюс Pydantic гарантируют валидный объект состояния. Ни регулярок, ни расхождений со схемой, ни выдуманных slot.
...
> 🎒 **На пальцах.** Функция JGA — три строки, и в них весь смысл метрики: словари сравниваются целиком, через `p == g`. Если из 100 turn полностью верны 80, JGA равен 0.80. Один лишний slot в предсказании — turn уже неверен, хотя все остальные значения совпали.
...
Заметив правку, перезаписывайте последний изменённый slot, а не дописывайте к нему. Без помощи LLM это сложно сделать аккуратно. Современный приём: пусть LLM каждый раз заново собирает всё состояние из истории, а не обновляет его по кусочкам — тогда правки обрабатываются сами собой.

> 🎒 **На пальцах.** Список подсказок для правок — четыре фразы: actually, no wait, on second thought, change that to. Расширить его несложно, но принцип из последнего абзаца надёжнее: пересобирайте состояние с нуля по всей истории. Тогда «actually make it moderate» обработается само, без всякого списка слов.
