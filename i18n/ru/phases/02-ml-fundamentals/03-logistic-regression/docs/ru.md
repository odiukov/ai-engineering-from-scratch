...
> 🎒 **На пальцах.** Термометр показывает «сколько градусов» — это регрессия. Врач говорит «температура есть или нет» — это классификация. Логистическая регрессия делает второе, но честно сообщает, насколько она уверена.
...
> 🎒 **На пальцах.** Схема читается слева направо: посчитали `wx + b`, сжали сигмоидой в вероятность, сравнили с 0.5, выдали ответ. Решением число становится только на последнем шаге. Всё, что до него, модель делает одинаково при любом пороге.
