...
> 🎒 **На пальцах.** Bias — это когда вы стабильно мажете в одну и ту же сторону. Variance — когда вы мажете каждый раз по-разному. Первое лечится другим оружием, второе — тренировкой. Весь урок про то, как отличить одно от другого.
...
> 🎒 **На пальцах.** Попробуйте описать форму горки одной прямой линией. Как ни двигайте линейку, вершина горки останется выше, а края ниже. Ошибка не уйдёт ни на обучении, ни на тесте — и обе будут примерно одинаковыми. Ровный и высокий уровень ошибки на обоих наборах — подпись bias.
...
> 🎒 **На пальцах.** Прочитайте таблицу как «горку наоборот»: слева ошибка высокая, посередине минимальная, справа снова высокая. Именно поэтому нельзя просто «сделать модель посложнее и станет лучше» — можно перевалить через дно и поехать вверх по правому склону.
...
> 🎒 **На пальцах.** Регуляризация — это как ограничение скорости. Вы сознательно едете медленнее оптимального (растёт bias), зато перестаёте вылетать на поворотах (падает variance). Ручка lambda: 0.001 — почти без ограничений, 100 — модель почти константа.
...
> 🎒 **На пальцах.** Списки специально не пересекаются: почти каждое лекарство от bias — яд для variance, и наоборот. «Больше данных» помогает только при variance. «Более сложная модель» помогает только при bias. Поэтому сначала диагноз, потом лечение, а не наоборот.
...
> 🎒 **На пальцах.** Два подхода — две разные ручки на одном приборе. Одна крутит количество примеров, вторая — сложность модели. Крутить их одновременно нельзя: если поменять и то и другое, вы не поймёте, что именно повлияло на результат. Ровно так же ставят школьный опыт: меняем одно, остальное фиксируем.
...
> 🎒 **На пальцах.** Степень полинома — переключатель с делениями: 5, 6, 7, промежуточного не бывает. Alpha — плавная ручка громкости: между 0.05 и 0.1 есть сколько угодно значений. Поэтому и берут одну большую степень плюс alpha, а не подбирают степень вручную.
