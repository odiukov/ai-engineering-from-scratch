...
> 🎒 **На пальцах.** Обычная программа при поломке падает и показывает пальцем на строку. Нейросеть — как ученик, который решил задачу и уверенно написал неверный ответ: тетрадь заполнена, почерк аккуратный, ошибку видно только если перерешать самому. Главный приём из этого списка занимает 30 секунд: взять 8 примеров, покрутить на них 100 шагов и посмотреть, упал ли loss почти в ноль.
