...
> 🎒 **На пальцах.** У ResNet-18 `fc.in_features` равно 512. Голова на 10 классов — это матрица 512×10 плюс 10 смещений, то есть 5130 параметров. Весь backbone — около 11 миллионов. Вы обучаете 0.05% модели и получаете 80% точности.
...
> 🎒 **На пальцах.** Типичная пара на CIFAR-10: 70% с замороженным backbone и 93% после fine-tuning. Разрыв 23 пункта — это то, что вы купили за размораживание. Если после fine-tuning стало 65%, вы не дообучили модель, а сломали её: почти всегда виноват слишком большой LR на backbone.
...
Пять эпох по этому рецепту на CIFAR-10 поднимают `ResNet18-IMAGENET1K_V1` с ~70% точности линейного пробинга до ~93% после fine-tuning. Одна голова без backbone упёрлась бы в потолок около 86%.

> 🎒 **На пальцах.** Три числа стоит запомнить: 70% — линейный пробинг, 86% — потолок обученной головы, 93% — полный fine-tuning. Каждая ступень стоит дороже предыдущей по вычислениям. Если вам хватает 86%, вы экономите обучение 11 миллионов параметров.
