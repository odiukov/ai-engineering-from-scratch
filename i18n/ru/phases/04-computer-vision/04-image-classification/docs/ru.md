...
Reflect-pad перед обрезкой, а не zero-pad, потому что чёрные рамки — это сигнал, который модель научится игнорировать бесполезным способом. Обе augmentation берут случайность из явного `np.random.Generator`, который вы передаёте снаружи, а не из глобального `np.random`, — та же дисциплина, что и в `synthetic_cifar` из Step 1, где генератор строится внутри из аргумента `seed`. Здесь это важно вот почему: единственный seed, который выставляет этот файл, — `torch.manual_seed(0)`, а он не трогает глобальный генератор NumPy. Потянетесь тут за `np.random.uniform` — и запуск молча перестанет быть воспроизводимым, пока вы не вспомните засеять ещё и NumPy отдельно.
