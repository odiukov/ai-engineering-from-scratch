...
2. Сэмплируем шаг `t` равномерно из [1, T].
...
`eta=0` полностью детерминирован (один и тот же входной шум всегда даёт один и тот же результат). `eta=1` возвращает DDPM.

> 🎒 **На пальцах.** `torch.linspace(T - 1, 0, steps + 1)` при `steps=50` строит лесенку 999, 979, 959 … 0 — то есть шагает через каждые 20 шагов расписания вместо каждого. Детерминированность из `eta=0` — это то, почему в веб-интерфейсах есть поле seed: одно и то же число даёт побайтово ту же картинку.
