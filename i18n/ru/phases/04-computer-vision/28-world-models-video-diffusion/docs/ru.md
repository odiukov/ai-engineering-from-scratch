...
1. **(Easy)** Посчитайте число токенов для 5-секундного видео 360p при 30 fps (150 кадров 640x360) с patch-t=2, patch-h=8, patch-w=8. Прикиньте, сколько памяти займёт внимание на таком размере.
...
> 🎒 **На пальцах.** Подсказка к первому заданию: все числа уже даны в условии — 150 кадров размером 640x360. Токенов получается (150/2) × (360/8) × (640/8) = 75 × 45 × 80 = 270000. Матрица полного внимания — это 270000², то есть примерно 7,3 × 10^10 чисел: даже в fp16 по 2 байта это около 146 ГБ на одну голову одного слоя. В H100 влезает 80 ГБ — то есть не хватает почти вдвое, и это ещё одна голова одного слоя. Отсюда и весь смысл divided attention.
