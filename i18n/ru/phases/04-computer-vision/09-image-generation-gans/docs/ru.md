...
Перед сэмплированием всегда переключайтесь в режим eval. Для DCGAN это важно, потому что тогда batch norm использует накопленную статистику вместо статистики текущего батча.
...
Замена BN в discriminator «как есть», гарантирующая, что сеть является 1-липшицевой. Чинит большинство отказов вида «D побеждает слишком уверенно».
...
> 🎒 **На пальцах.** «1-липшицева» означает, что изменение входа на единицу меняет выход не больше чем на единицу — сеть не может стать сколь угодно крутой. Discriminator без такого ограничения превращается в отвесную стену: он отвечает 0.999 и 0.001, а generator получает нулевой градиент и стоит на месте. Spectral norm делит веса каждого слоя на их наибольшее сингулярное число, ограничивая наклон. Обратите внимание, что в этом варианте BatchNorm исчез совсем.
