...
В типичной сцене 1-5 миллионов гауссиан. Каждая хранит примерно 60 float (3 + 4 + 3 + 1 + 48 + мелочь). Это 240 МБ на сцену из пяти миллионов гауссиан — заметно меньше, чем эквивалентное облако точек с текстурой на каждую точку, и на порядок меньше, чем веса MLP у NeRF в высоком разрешении.

> 🎒 **На пальцах.** Сложите числа из блока сами: 3 (позиция) + 4 (кватернион) + 3 (масштаб) + 1 (непрозрачность) + 48 (цвет) = 59. Почти всё место съедает цвет — 48 из 59, то есть больше 80%. Если снизить степень SH с 3 до 0, останется 3 коэффициента вместо 48, и гауссиана похудеет с 59 float до 14. Расплата — цвет перестанет меняться с ракурсом, блики исчезнут.
...
Базис SH до степени 3 включительно содержит 16 членов на канал. Вычисление:
