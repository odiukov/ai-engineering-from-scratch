...
- Замерить пайплайн целиком и найти первое узкое место (детектор — и на CPU, и на GPU, но на GPU доля препроцессинга в общем времени становится куда заметнее)
...
1. **Preprocessing is easy to underestimate.** Декодирование JPEG, перевод цветовых пространств, изменение размера — всё это упирается в CPU и легко забывается. На машине без GPU детектор перевешивает препроцессинг с большим отрывом (числа — в шаге 5). Переезд моделей на GPU не ускоряет препроцессинг вообще никак, поэтому его *доля* в общем времени резко растёт, хотя абсолютная стоимость не меняется, — это та часть пайплайна, которую более быстрая видеокарта не чинит.
