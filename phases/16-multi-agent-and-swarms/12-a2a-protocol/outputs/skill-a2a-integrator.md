...
1. **Agent Card.** Name, description, agent version, `supportedInterfaces` (binding + protocol version), capabilities, default input/output media types, `AgentSkill` objects, and security declarations.
...
6. **Idempotency.** Strategy for duplicate `POST /message:send` requests using client-created `messageId`; new task IDs remain server-assigned.
